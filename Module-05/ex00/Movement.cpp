...
Movement::Movement(int p_weight, double p_friction, double p_accelerate, double p_brake):
...
void Movement::_maintainMove() {
	_acceleration = -_frictionForce / _mass;
	_updateVelocity();
	_currentState = MAINTAINING;
}

void Movement::_stopMove() {
	_velocity = 0;
	_acceleration = 0;
	_currentState = STOPPED;
}

void Movement::_updateVelocity() {
...
void Movement::move(TrainState event) {
