...
: AMovement(p_weight, p_friction, p_acceleration, p_brake), AClock(), _ID(_nextID++), _name(p_name), _departureStation(p_departure),
	_arrivalStation(p_arrival), _pathIndex(0), _hasStart(false), _departureTime(p_depTime), _stopTime(p_stopTime)
{
...
void Train::update(long int p_dt) {
	...
	double tDeccel = (getBrakeMax() + getFrictionCoefficient()) / getWeight();
	...
	//TODO: le chrono de stopTime;
	//TODO: Actuellement on s'arrete a tous les aiguillage meme si c'est pas une station 
